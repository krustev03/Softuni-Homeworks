    ...
    public class Program
    {
        public static void Main()
        {
            ...
            while (command != "Output")
            {
                ...
                bool haveSpace = departments[department].SelectMany(x => x).Count() < 60;
                if (haveSpace)
                {
                    int room = 0;
                    doctors[fullName].Add(patient);
                    for (int currRoom = 0; currRoom < departments[department].Count; currRoom++)
                    {
                        if (departments[department][currRoom].Count < 3)
                        {
                            room = currRoom;
                            break;
                        }
